...
public class MapperFileFormatter extends Formatter implements IFormatter {
    @Override
    public void format(FormatterArgs args) {

        String template = null;
        try {
            template = fileReaderUtil.read(FileEnum.MAPPER.templateFilename);
        } catch (IOException e) {
        ...
        String path = String.format(FileEnum.MAPPER.outputFilepathFormat, args.getOutputDir(), args.getPackageName(), args.getPascalName());
    ...
    public static void main(String[] args) {
        ...
        new MapperFileFormatter().format(formatterArgs);
